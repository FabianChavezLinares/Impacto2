...
if (!empty($_POST)) {
  ...
  if (empty($_POST['codigo']) || empty($_POST['producto']) || empty($_POST['precio'])) {
    ...
    $query_update = mysqli_query($conexion, "UPDATE producto SET codigo = '$codigo', descripcion = '$producto', categoria='$categoria', precio= $precio WHERE codproducto = $codproducto");
    if ($query_update) {
      $alert = '<div class="alert alert-primary" role="alert">
              Producto Modificado
            </div>';
      ...
      <div class="card-body">
        <form action="" method="post">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" placeholder="Ingrese la categoria" name="categoria" id="categoria" value="<?php echo $data_producto['categoria']; ?>">
          </div>
          <div class="form-group">
